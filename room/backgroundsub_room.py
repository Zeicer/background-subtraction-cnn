...
def get_data_loader(data_dir, split_name, batch_size=128):
    max_samples = 2560000 if split_name == "train" else 512000
...
# 模型訓練流程
def train_model(environment1 = 'room', environment2 = 'room_patch'):
    data_dir = f'./dataset/{environment1}/{environment2}'  # 注意路徑修正

    # 1. 兩個 DataLoader
    train_loader, train_dataset = get_data_loader(data_dir, "train", batch_size=128)
    valid_loader, valid_dataset = get_data_loader(data_dir, "valid", batch_size=128)
