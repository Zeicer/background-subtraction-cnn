...
def build_yolo_foreground_with_mask(
    ori_img,
    person_bgs_mask,
    white_block_threshold=4,
    expand_pixels=5
):
    """
    依照白塊數量決定 YOLO 前的背景剪除方式：
    - 白塊數量 < white_block_threshold：直接用原本 mask 做背景剪除
    - 白塊數量 >= white_block_threshold：先將白塊向外延伸 expand_pixels 像素，再做背景剪除
    """
    ...
    if white_block_count < white_block_threshold:
        yolo_mask = person_bgs_mask.copy()
        mask_mode = "direct"
    else:
    ...
    return foreground_only, yolo_mask, white_block_count, mask_mode
...
def main(
    ...
    for path in image_paths:
        ...
        foreground_only, yolo_person_mask, white_block_count, mask_mode = (
            build_yolo_foreground_with_mask(
                ori_img,
                person_bgs_mask,
                white_block_threshold=4,
                expand_pixels=10
        ...
        # ✅ YOLO 判斷前一定先做背景剪除：
        # 白塊數量 < 4：直接用原 mask 剪背景。
        # 白塊數量 >= 4：先將白塊向外延伸 10 像素，再剪背景。
        # 注意：等於 4 時也歸入延伸處理，避免條件空窗。
