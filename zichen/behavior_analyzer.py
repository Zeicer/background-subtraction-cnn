...
# 🚶 3. 徘徊判定：頭腳高度比例（80%~110%）+ 頭部點位移在固定範圍內（支援 deque 修正版）
    ...
    def check_loitering(self, pid, range_threshold=55, min_frames=90):
        ...
        # 1. 走訪最近 3 秒，蒐集頭腳距離與頭部座標
        for i in range(len(recent_tracks)):
            _, _, _, _, kpts = recent_tracks[i]
            ...
            if head and foot:
                dist = np.sqrt((head[0] - foot[0])**2 + (head[1] - foot[1])**2)
        ...
        # 這裡從 track[-1] 拿最後一幀沒問題（deque 支援單一整數索引）
        _, _, _, _, kpts_now = track[-1]
        head_now, foot_now = self._get_head_foot_data(kpts_now)
        
        if head_now and foot_now and avg_height > 0:
            current_height = np.sqrt((head_now[0] - foot_now[0])**2 + (head_now[1] - foot_now[1])**2)
        ...
        # 3. 條件二：幾何核心，計算最新 3 秒內頭部點移動的外包邊界
